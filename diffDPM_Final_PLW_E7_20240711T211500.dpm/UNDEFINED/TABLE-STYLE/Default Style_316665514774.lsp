
(GRID::TABLE-STYLE
 :OBJECT-NUMBER 316665514774
 :NAME "Default Style"
 :ADVANCED-STYLE T
 :CONTEXT-DEPENDENCIES ((:CURVE-TYPE10 :ARCHIVE-PREDICTED-COST)
                        (:CURVE-TYPE9 :ESTIMATE-TO-COMPLETE)
                        (:CURVE-TYPE8 :EXPENDITURE)
                        (:CURVE-TYPE7 :BUDGET)
                        (:CURVE-TYPE6 :PREDICTED-COST)
                        (:TIME-UNIT-FORM5 NIL) (:TIME-UNIT-FORM4 NIL)
                        (:TIME-UNIT-FORM3 #.(:ORF'^TUF"WEEK:2C"))
                        (:TIME-UNIT-FORM2 #.(:ORF'^TUF"MONTH:3C"))
                        (:TIME-UNIT-FORM1 #.(:ORF'^TUF"YEAR:4C"))
                        (:DATE-FORMAT5
                         #.(:ORF'^DFT"JJ-MMM-AAAA:HHhIImSSs"))
                        (:DATE-FORMAT4
                         #.(:ORF'^DFT"JJ/MM/AA:HHhIImSSs"))
                        (:DATE-FORMAT3
                         #.(:ORF'^DFT"JJ/MM/AA:HHhIImSSs"))
                        (:DATE-FORMAT2
                         #.(:ORF'^DFT"JJ/MM/AA:HHhIImSSs"))
                        (:DATE-FORMAT1 #.(:ORF'^DFT"JJ-MMM-AA"))
                        (:DURATION-FORMAT5 #.(:ORF'^DUFT"JHMS"))
                        (:DURATION-FORMAT4 #.(:ORF'^DUFT"JHMS"))
                        (:DURATION-FORMAT3 #.(:ORF'^DUFT"JHMS"))
                        (:DURATION-FORMAT2 #.(:ORF'^DUFT"JHMS"))
                        (:DURATION-FORMAT1 #.(:ORF'^DUFT"D"))
                        (:NUMBER-FORMAT5 #.(:ORF'^NFT"standard+"))
                        (:NUMBER-FORMAT4 #.(:ORF'^NFT"standard+"))
                        (:NUMBER-FORMAT3 #.(:ORF'^NFT"#,###.00"))
                        (:NUMBER-FORMAT2 #.(:ORF'^NFT"#,###.0"))
                        (:NUMBER-FORMAT1 #.(:ORF'^NFT"#,###"))
                        (:DATE5 NIL)
                        (:DATE4
                         (KERNEL-ORDO:FIND-TYPE-DATE-FROM-NAME
                           :BUDGET))
                        (:DATE3 NIL) (:DATE2 NIL) (:DATE1 NIL)
                        (:CURVE-TYPE5 :PREDICTED-COST)
                        (:CURVE-TYPE4 :BUDGET)
                        (:CURVE-TYPE3 :EXPENDITURE)
                        (:CURVE-TYPE2 :PREDICTED-COST)
                        (:CURVE-TYPE1 :BUDGET)
                        (:TIME-UNIT5
                         #.(:ORF'#%TIME-TYPES:TIME-UNIT:"DAY"))
                        (:TIME-UNIT4
                         #.(:ORF'#%TIME-TYPES:TIME-UNIT:"WEEK"))
                        (:TIME-UNIT3
                         #.(:ORF'#%TIME-TYPES:TIME-UNIT:"MONTH"))
                        (:TIME-UNIT2
                         #.(:ORF'#%TIME-TYPES:TIME-UNIT:"QUARTER"))
                        (:TIME-UNIT1
                         #.(:ORF'#%TIME-TYPES:TIME-UNIT:"YEAR"))
                        (:UNIT5 #.(:ORF'#%TIME-TYPES:TIME-UNIT:"HOUR"))
                        (:UNIT4
                         #.(:cft'DOD:COST-UNIT "Default cost unit" 0))
                        (:UNIT3 #.(:ORF'#%TIME-TYPES:TIME-UNIT:"DAY"))
                        (:UNIT2 #.(:ORF'#%TIME-TYPES:TIME-UNIT:"DAY"))
                        (:UNIT1
                         #.(:cft'DOD:COST-UNIT "Default cost unit" 0))
                        (:_INF_AA_B_ADMIN_MODE T)
                        (:_SC_AA_B_DISPLAY_FAVORITES NIL)
                        (:_RE_AA_B_MENU_CLOSED NIL)
                        (:_GUI_AA_B_DISPLAY_TREEVIEW T)
                        (:*APPLY-63-LOOK-AND-FEEL-AT-STARTUP* T)
                        (:_ADM_AA_S_DM_CREATED_OBJECT "")
                        (:_ADM_AA_S_OC_CONF_DATASET_CLASS "")
                        (:_GUI_AA_B_P6_NAVIGATION_MODE T)
                        (:_GS_AA_S_USER_SETTINGS_FILE "GLOBALSETTINGS"))
 :DATASET 118081000141
 :GROUP-READ-ONLY (#.(:UNFINALIZED 'GENERIC-IO:GROUP-OR-USER '"OR_FUNCT_ADM_PHARMA" NIL)
                   #.(:UNFINALIZED 'GENERIC-IO:GROUP-OR-USER '"OR_FUNCT_ADM_VACCINES" NIL)
                   #.(:UNFINALIZED 'GENERIC-IO:GROUP-OR-USER '"OR_FUNCT_ADM_CONTINUUM" NIL))
 :GROUP-READ-WRITE (#.(:UNFINALIZED 'GENERIC-IO:GROUP-OR-USER '"R_ITS_ADMIN" NIL))
 :IGNORE-DEPENDENCIES T
 :IGNORED-DEPENDENCIES (:UNIT1 :UNIT2 :UNIT3 :UNIT4 :UNIT5 :TIME-UNIT1
                        :TIME-UNIT2 :TIME-UNIT3 :TIME-UNIT4 :TIME-UNIT5
                        :DATE1 :DATE2 :DATE3 :DATE4 :DATE5
                        :NUMBER-FORMAT1 :NUMBER-FORMAT2 :NUMBER-FORMAT3
                        :NUMBER-FORMAT4 :NUMBER-FORMAT5
                        :DURATION-FORMAT1 :DURATION-FORMAT2
                        :DURATION-FORMAT3 :DURATION-FORMAT4
                        :DURATION-FORMAT5 :DATE-FORMAT1 :DATE-FORMAT2
                        :DATE-FORMAT3 :DATE-FORMAT4 :DATE-FORMAT5
                        :TIME-UNIT-FORM1 :TIME-UNIT-FORM2
                        :TIME-UNIT-FORM3 :TIME-UNIT-FORM4
                        :TIME-UNIT-FORM5)
 :ON-CLASS OPX2-USER::__USER_TABLE_SAN_RDPM_UT_CLIN_MS_RULES
 :OTHER-DATA (:OBJECT-GRID
              (:COLUMN-GROUPS
               ((:USER_ATTRIBUTE_SAN_RDPM_UA_S_MILESTONE_TYPE
                 "Clinical Milestone definition")
                (:USER_ATTRIBUTE_SAN_RDPM_UA_S_FUNC_DESC
                 "Clinical Milestone definition")
                (:USER_ATTRIBUTE_SAN_RDPM_UA_S_LEVEL
                 "Clinical Milestone definition")
                (:USER_ATTRIBUTE_SAN_RDPM_UA_S_CLIN_MS
                 "Clinical Milestone definition")
                (:USER_ATTRIBUTE_SAN_RDPM_UA_S_ACT_TYPES
                 "Computation rules")
                (:USER_ATTRIBUTE_SAN_RDPM_UA_B_IGNORE_AT_ORDER
                 "Computation rules")
                (:USER_ATTRIBUTE_SAN_RDPM_UA_S_DATE_SLOT
                 "Computation rules")
                (:USER_ATTRIBUTE_SAN_RDPM_UA_S_ORDER_DATE
                 "Computation rules")
                (:USER_ATTRIBUTE_SAN_RDPM_UA_S_OPERATION
                 "Computation rules")))
              (:ATTRIBUTES
               (:USER_ATTRIBUTE_SAN_RDPM_UA_S_APPLY_TO
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_CLIN_MS
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_LEVEL
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_MILESTONE_TYPE
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_FUNC_DESC
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_OPERATION
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_ORDER_DATE
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_DATE_SLOT
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_ACT_TYPES
                :USER_ATTRIBUTE_SAN_RDPM_UA_B_IGNORE_AT_ORDER
                :USER_ATTRIBUTE_SAN_RDPM_UA_B_GENERATE_FIELD
                :USER_ATTRIBUTE_SAN_RDPM_UA_B_EXPORT
                :USER_ATTRIBUTE_SAN_RDPM_UA_B_IMPORT :DATASET))
              (:SORT (:USER_ATTRIBUTE_SAN_RDPM_UA_S_CLIN_MS))
              (:GROUP-BY
               ((:USER_ATTRIBUTE_SAN_RDPM_UA_S_APPLY_TO
                 #.(:gc ':_GPL1 ':_GPF1))
                (:USER_ATTRIBUTE_SAN_RDPM_UA_S_CLIN_MS_RULE_TYPE
                 #.(:gc ':_GPL2 ':_GPF2))))
              (:REPORT-MODE-NUMBER-OF-COLUMNS 1)
              (:DISPLAY-SUM-AT-END 0)
              (:DISPLAY-REPORT-GROUP-AND-SORT 1) (:INDENT-CELLS 1)
              (:NUMBER-OF-REPORTS-PER-PAGE 50)
              (:REPORT-DISPLAYED-RW NIL) (:REPORT-DISPLAYED-RO NIL)
              (:REPORT-MODE 0) (:THUMBNAIL-LABEL-POSITION :TOP)
              (:THUMBNAIL-TOOLTIP-FORMULA
               #.(:F"NOTE_PAD":STRING'#%OBJECT:EMPTY-CLASS:))
              (:THUMBNAIL-LABEL-FORMULA
               #.(:F"ID":STRING'#%OBJECT:EMPTY-CLASS:))
              (:THUMBNAIL-IMAGE-FIELD NIL) (:THUMBNAIL-SIZE :64X64)
              (:THUMBNAIL-MODE 0) (:DISPLAY-IMAGE-FIELDS 0)
              (:MAX-LINE-HEIGHT 100) (:DISPLAY-RICH-TEXT 0)
              (:ADJUST-LINE-HEIGHT-TO-CONTENT 0) (:SHELL-HEIGHT 731)
              (:SHELL-WIDTH 1536) (:READ-WRITE-COLOR :WHITE)
              (:READ-ONLY-COLOR :ROCC) (:LABEL-SUPRESSED-COLOR :GREY)
              (:LABEL-MODIFIED-COLOR :DARK-GREEN)
              (:LABEL-DEFAULT-COLOR :BLACK) (:LABEL-COLOR :RED)
              (:LABEL-FORMULA
               #.(:F"FALSE":BOOLEAN'#%OBJECT:EMPTY-CLASS:))
              (:GROUP-LEVEL 0) (:GROUP-LEVEL-OPERATOR >=)
              (:LEVEL-LIMITATION-NUMBER 0)
              (:LEVEL-LIMITATION-OPERATION >=) (:DRAW-COLUMNS 0)
              (:DRAW-LINES 1)
              (:FONT #{GRAPHIC-TYPES:STANDARD-FONT@FONT9.9})
              (:FORMULA #.(:F"[TRUE]":BOOLEAN'#%OBJECT:EMPTY-CLASS:))
              (:SHOW-OBJECT-ICONS 0) (:SHOW-HIDDEN-OBJECTS 0)
              (:SPACE-BETWEEN-GROUP 12)
              (:OBJECT-CLASS
               OPX2-USER::__USER_TABLE_SAN_RDPM_UT_CLIN_MS_RULES)
              (:FOLLOW-SUBCLASSES T)
              (:LABEL-FONT #{GRAPHIC-TYPES:STANDARD-FONT@FONT9.9})
              (:GROUP-FONT #{GRAPHIC-TYPES:STANDARD-FONT@FONT10.9})
              (:MERGE-GROUP-BARS 1) (:INITIAL-GROUP-LEVEL NIL)
              (:COLOR1 :LINEV) (:COLOR2 :LINOD) (:ALTERNATE-COLORS 0)
              (:ALTERNATE-BACKGROUND :REPBK)
              (:DISPLAY-PARENTS-OF-FILTERED-OBJECTS 0)
              (:FORCE-GROUP-MERGE 0) (:DISPLAY-GROUP-OBJECTS 0)
              (:ATTRIBUTE-WIDTH-PLIST
               (:USER_ATTRIBUTE_SAN_RDPM_UA_S_LEVEL 14
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_APPLY_TO 13
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_ORDER_DATE 12
                :USER_ATTRIBUTE_SAN_RDPM_UA_B_IGNORE_AT_ORDER 8
                :COMMENT 20
                :USER_ATTRIBUTE_SAN_RDPM_UA_B_GENERATE_FIELD 10
                :NOTE-PAD 50 :USER_ATTRIBUTE_SAN_RDPM_UA_S_ACT_TYPES 37
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_DATE_SLOT 14
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_OPERATION 12
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_CLIN_MS 20))
              (:SUMAV-TO-DO NIL) (:SPLIT-INFO NIL)
              (:FIXED-WIDTH-ATTRIBUTES
               (:USER_ATTRIBUTE_SAN_RDPM_UA_S_ORDER_DATE T
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_LEVEL T
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_APPLY_TO T :NOTE-PAD T
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_ACT_TYPES T
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_DATE_SLOT T
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_OPERATION T
                :USER_ATTRIBUTE_SAN_RDPM_UA_S_CLIN_MS T))
              (:CHANGED-FORMATS NIL))
 :OWNER 21610
 :TABLE-WIDTH 0
 :_IHM_AA_B_BYPASS_ADV_ST "0"
 :_IHM_DA_CARDSSTYLE-ONB 0
 :_IHM_P6_AA_N_ORDER -1.0d0
)