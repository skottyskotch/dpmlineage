...
(JAVASCRIPT::USER-SCRIPT
 :OBJECT-NUMBER 321303458582
 :NAME "SAN_RDPM_JS2_EXPORT_REPORT_UTILS"
 :COMMENT "Functions to manage heavy report export by batch"
 :ACTIVE T
 :DATASET 118081000141
 :EVAL-ON-LOAD T
 :LOAD-ORDER 0
 :SCRIPT-CODE "namespace _san_launch_batch;
function launch_batch_macro_export_pex(batch_identifier_customer_settings,min_rest_duration_before_launch,impex_target_name,retention_duration_days){
	...
	if (o_main_batch instanceof plc._INF_PT_REQ){
		...
		var b_is_recent_batch = false;
		if (batch_last_running_date != undefined && batch_last_running_date InstanceOf Date) {
			b_is_recent_batch=(batch_last_running_date + (min_rest_duration_before_launch*60)) > new date();
		}
		...
		if (b_is_recent_batch == true || o_main_batch._BA_AA_B_RUN_BATCH == true) {
			plw.alert(\"The export is already running or has been launched too recently (less than \"+min_rest_duration_before_launch+\" minutes after the previous launch).\");
			} else {
...
function launch_batch_macro_export_pex_wpa_report() {
	var integer min_rest_duration_before_launch = context.SAN_UA_RDPM_N_DELAY_WPA_BATCH;
	var string batch_identifier_customer_settings = \"SAN_RDPM_CS_WPA_BATCH_NUMBER\";
	var string impex_target_name =\"SAN_IMPEX_TARGET_WPA_EXPORT:CSV file format\";
	var retention_duration_days = 1;
	launch_batch_macro_export_pex(batch_identifier_customer_settings,min_rest_duration_before_launch,impex_target_name,retention_duration_days);
}
"
