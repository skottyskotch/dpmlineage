...
(JAVASCRIPT::USER-SCRIPT
 :OBJECT-NUMBER 319716168069
 :NAME "SAN_RDPM_JS2_PRINT_MONTHLY_REPORT"
 :COMMENT "Monthly report printing"
 :ACTIVE T
	...
	if (vPrez!=undefined)
	{
		if (vPortfolio!=undefined)
		{
			...
			for (var vProj in vListProj)
			{
				...
				if (vFranch!=undefined && vFranch!=\"\" && vFranch.internal==false)
				{
					...
					else
					{
						...
						if (vPrezFra!=undefined)
						{
							...
							// PC-6925 new argument to drive option \"Scaling\" when printing a presentation
							var vPDFfileFra = vPrezFra.printpresentation(vProj,\"PDF-PRINTER\",FIT_CHART_TO_PAGE : #BOTH#);
				...
				// PC-6925 new argument to drive option \"Scaling\" when printing a presentation
				var vPDFfile = vPrez.printpresentation(vProj,\"PDF-PRINTER\",FIT_CHART_TO_PAGE : #BOTH#);
