...
(JAVASCRIPT::USER-SCRIPT
 :OBJECT-NUMBER 262077656373
 :NAME "SAN_JS2_DATAHUB_IMPORT"
 :COMMENT "Datahub import batch, used in SAN_BA_DATAHUB_IMPORT"
 :ACTIVE T
...
// PC-6920 : Add a step in datahub import batch to delete old events (19-JUL-2023 ep)
function san_js2_delete_old_impex_event(target_id) {
	var count_event=0;
	var lastdate = plw.periodstart(new date(),\"DAY\",-1*CONTEXT._IMPEX_ST_N_DAY_OLD_TO_CLEAN);
	var impt_obj=plc.impex_target.get(target_id);
	for (var ev in plc.impex_event where ev.target==impt_obj && ev.date<=lastdate) {
		with(ev.fromobject()) {
			for (var err in plc.impex_error) {
				err.delete();
			}
		}
		count_event++;
	...
	plw.writetolog(\"san_js2_delete_old_impex_event: deleted \"+count_event+\" event before \"+lastdate.tostring(\"DD/MM/YYYY\"));
	return count_event;
}

// deleting old events
plw.writetolog(\"Deleting old events...\");
var nb_event=_san_datahub_import.san_js2_delete_old_impex_event(\"SAN_IMPEX_TARGET_DATAHUB_IMPORT:JSON file format\");
plw.writetolog(\"Deleted \"+nb_event+\" old events...\");
// END PC-6920
