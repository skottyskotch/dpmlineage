...
(JAVASCRIPT::USER-SCRIPT
 :OBJECT-NUMBER 260067159772
 :NAME "SAN_RDPM_JS2_DYN_RELATION"
 :COMMENT "RDPM Dynamic Relations"
 :ACTIVE T
...
//  AUTHOR  : S. AKAAYOUS
//
//  v1.9 - 2022/06/01 - William
//  Fix PC-6079 san_rdpm_js2_tm_to_enable() to take into account the new group PORTFOLIO_CONTINUUM_TT_PROD for PHARMA and PORTFOLIO_CONTINUUM_RDPM_VAC_PROD for vaccins 
//
...
function san_rdpm_js2_tm_to_enable(f){
	// PC-6079 add PORTFOLIO_CONTINUUM_RDPM_VAC_PROD to the control
	// Verifier profile P_TC_ONLY puis si P_STD vérifier les groupes
	var v_user_groups = ['M_VACCINES','O_GBU_VACCINES'];
				...
				for(var o_tm in plc.__USER_TABLE_SAN_RDPM_UT_PROJECT_TEAM_MEMBER where o_tm.get('SAN_RDPM_UA_PROJECT_ROLE')!=''){	
					if(o_tm.get('SAN_RDPM_UA_PROJECT_ROLE').get('SAN_RDPM_UA_PR_B_PRJ_READ_PERMISSION')){
						...
						if(o_tm_user instanceof plc.opx2_user){
							// PC-6079 modification to check profile first
							var b_tm_to_manage = false;
							if(o_tm_user.callbooleanformula('USER_IN_GROUP(ID,\"P_TC_ONLY\")')){b_tm_to_manage=true;}
							//Check user groups of the project team member
							if (!b_tm_to_manage) {
								if(o_tm_user.callbooleanformula('USER_IN_GROUP(ID,\"P_STD\")')) {
									for(var s_user_group in v_user_groups){
										if(o_tm_user.callbooleanformula('NOT USER_IN_GROUP(ID,\"'+s_user_group+'\")')){b_tm_to_manage=true;}
									}
									//Check the other properties
									if (!b_tm_to_manage) {
										var v_user_groups_t = ['P_STD','M_VACCINES','O_GBU_VACCINES','PORTFOLIO_CONTINUUM_RDPM_VAC_PROD'];
										// PC-6079
										if(o_tm_user.GROUPS_LIST.split(',').length==4 && o_tm_user.GROUPS_LIST.split(',').difference(v_user_groups_t)==false){
...
function san_rdpm_js2_tm_to_disable(f){
	// PC-6079 add PORTFOLIO_CONTINUUM_RDPM_VAC_PROD to the control
	var v_user_groups = ['P_STD','M_VACCINES','O_GBU_VACCINES','PORTFOLIO_CONTINUUM_RDPM_VAC_PROD'];
	...
	if(context.callbooleanformula('USER_IN_GROUP($CURRENT_USER,\"OR_FUNCT_ADM_VACCINES\")')){
		for(var o_user in plc.opx2_user where o_user.callbooleanformula('USER_IN_GROUP(ID,\"O_GBU_VACCINES\")') && o_user.get('OPX2_INTRANET_ACCESS')){
			// PC-6079 modification from 3 to 4 because there is 4 mandatory groups
			if(o_user.GROUPS_LIST.split(',').length==4 && o_user.GROUPS_LIST.split(',').difference(v_user_groups)==false){
