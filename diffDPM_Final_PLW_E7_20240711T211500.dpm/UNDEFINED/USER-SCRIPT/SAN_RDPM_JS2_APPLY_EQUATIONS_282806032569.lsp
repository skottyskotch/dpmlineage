...
(JAVASCRIPT::USER-SCRIPT
 :OBJECT-NUMBER 282806032569
 :NAME "SAN_RDPM_JS2_APPLY_EQUATIONS"
 :COMMENT "Apply equations on R&D projects"
 :ACTIVE T
...
//
//  Revision 2.0 2024/05/31 Ahenry
//  More traces on batch creation
//
		...
		if (vListofProj!=undefined && vListofProj!=\"\")
		{
    		...
    		*/
    		...
    		if (vBatch!=undefined)
    		{
				...
				plw.writetolog(\"Batch \"+vBatch.printattribute()+\" updated!\");
    		}
    		else
    		{
				...
				plw.writetolog(\"Batch \"+vBatch.printattribute()+\" created!\");
    		}
