...
(JAVASCRIPT::USER-SCRIPT
 :OBJECT-NUMBER 262044406270
 :NAME "SAN_RDPM_JS2_DYN_ATTRIBUTES"
 :COMMENT "RDPM Dynamic attributes"
 :ACTIVE T
...
//
//  Modification - GSE - 07/12/2022 : PC-6449 - Modification of function san_slot_reader for Skills issue
//  Modification - GSE - 07/12/2022 : PC-6449 - Creation of Dynamic attribute for TAs metrics - Addition of lines to create the DA in Planned hours table
...
function san_prj_prime_gov_slot_reader(s_prj_slot,s_prj_code_slot) {
	...
	if(o_prj instanceof plc.ordo_project){
    	if(o_prj.SAN_UA_RWE_PROJECT_CODE_PRIME instanceof plc.__USER_TABLE_SAN_RWE_UT_PROJECT_CODES && (o_prj.getinternalvalue('STATE').toString()=='ACTIVE' || o_prj._PM_NF_B_IS_A_VERSION==true)){
			...
			// PC-6948 : must test if o_parent_prj does exist / is visible / is not undefined
    	    if (o_parent_prj InstanceOf plc.ordo_project) {
