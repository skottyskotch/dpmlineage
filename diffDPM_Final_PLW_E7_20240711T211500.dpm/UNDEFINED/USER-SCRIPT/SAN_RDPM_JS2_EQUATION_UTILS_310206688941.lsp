...
(JAVASCRIPT::USER-SCRIPT
 :OBJECT-NUMBER 310206688941
 :NAME "SAN_RDPM_JS2_EQUATION_UTILS"
 :COMMENT "RDPM - functions on equations"
 :ACTIVE T
...
//
// v1.8 - 2022/09/14 - WST
// PC-6330 - OTD 139927 - Remove the break and add step to write the init_value to force the calculation of CF
//
//  v1.7 - 2022/06/29 - WST
// PC-6178 Add control on the expenditure type to delete only AUTO and MANUAL planned expenditure
//
//  v1.6 - 2022/04/01 - David
//  Add san_rdpm_js_clean_cancelled_studies and modify san_rdpm_js_apply_equation_on_list to add the deletion of PH/PE for cancelled Vaccines studies (PC-5683)
//
	...
	if (vAct instanceof plc.workstructure)
	{
		...
		for (var vCCid in vListCC.parsevector())
		{
			...
			if (vCC!=undefined)
			{
				...
				// OTD 139927
				plw.writetolog(\"CF init_value for act \"+vAct+\": \"+init_value);
				// OTD 139927
			}
		}
	}
...
// vVect is a list of project name
function san_rdpm_js_apply_equation_on_list(vVect)
{
	...
	for(var o_projname in vVect)
	{
		...
		if (o_proj!=undefined)
		{
			...
			{
				// OTD 139927
				plw.writetolog(\"Activity :\" + vAct+\" vAct.SAN_RDPM_UA_ACT_N_CALCULATED_SUBJECTS :\"+vAct.SAN_RDPM_UA_ACT_N_CALCULATED_SUBJECTS);
				// OTD 139927
			...
			// remove alloc/expend for Vaccines studies (PC-5683)
			if (o_proj.SAN_RDPM_B_RND_VACCINES_PROJECT==true) san_rdpm_js_clean_cancelled_studies(o_proj);
		}
...
// remove PH and PE on studies 'Clinical studies' Cancelled or On hold for Vaccines projects
// vProj is a project
function san_rdpm_js_clean_cancelled_studies(vProj)
{
	if (vProj instanceof plc.ordo_project)
	{
		var vToDel=new vector();
		plw.writetolog(\" - Starting cleaning cancelled/on-hold clinical studies on \"+vProj.name);
		for(var plc.workstructure vAct in vProj.ACTIVITIES where vAct.SAN_UA_RDPM_B_IS_A_STUDY && vAct.SAN_UA_ACT_B_CALCULATED_IN_HUMAN && vAct.SAN_RDPM_UA_B_CLIN_STUDY_HOLD_CANC==true)
		{
			var i=0;
			// get allocation from study and activities below
			with(vAct.fromobject())
			{
				for (var vAlloc in plc.task_alloc)
				{
					vToDel.push(vAlloc);
					i++;
				}
				// PC-6178 Add control on the expenditure type to delete only AUTO and MANUAL planned expenditure
				for (var vExp in plc.expenditure where (vExp.TYPE==\"AUTO\" || vExp.TYPE==\"MANUAL\"))
				{
					vToDel.push(vExp);
					i++;
				}
			}
			plw.writetolog(\" --- Study ~a to be cleaned : ~a allocations/expenditures to be deleted\".format(vAct.printattribute(),i.tostring()));
		}
		if (vToDel.length>0)
		{
			plw.writetolog(\" --- Deletion start\");
			with([plw.no_locking, plw.no_alerts])
			{
				vToDel.callmacro(\"REMOVE\");
			}
			plw.writetolog(\" --- Deletion end. Nb of deleted objects : \"+vToDel.length);
		}
		plw.writetolog(\" - End cleaning cancelled clinical studies on \"+vProj.name);
	}
}"
