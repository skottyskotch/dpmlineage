...
(JAVASCRIPT::USER-SCRIPT
 :OBJECT-NUMBER 282701597541
 :NAME "SAN_RDPM_JS2_INSERT_LIB_TOOLS"
 :COMMENT "RDPM Library insertion functions"
 :ACTIVE T
...
function san_js2_insert_library() {
	...
	// E7121 compliance. The feature doesn't handle selection anymore in the minigantt. It's a boolean field to tick instead.
	// var v_selection = plw.selection_vector();
	var v_selection = context.PM_TPLIB_POPUP_SELECTION.keys;;
	...
	if (o_actSelection InstanceOf plc.work_structure) {
		...
		if (b_indication_selected==false && b_no_selection_done==false) {
			...
			plw.writetolog(\"... Library inserted. Start managing Delegated to...\");
			
			// PC-6726 : retrieve Delegated to from the library
			with(o_actSelection.fromobject()){
				for (var o_actSyncTarget in plc.work_structure where o_actSyncTarget._PM_AA_N_LIBRARY_ACT_ONB!=0){
					...
					// PC-6944 added a control on o_actTemplate!=undefined
					if (o_actTemplate!=undefined) {
						if (o_actSyncTarget._WPM_RO_WORKPACKAGE_MANAGER==\"\") {
							o_actSyncTarget._WPM_RO_WORKPACKAGE_MANAGER=o_actTemplate._WPM_RO_WORKPACKAGE_MANAGER;
						}
					}
				}
			}
