...
(JAVASCRIPT::USER-SCRIPT
 :OBJECT-NUMBER 260029509872
 :NAME "SAN_RDPM_JS2_TC_UTILS"
 :COMMENT "Functions used for timecard to manage list of activities rdpm"
 :ACTIVE T
...
//
//  Modification 0.7 2024/04/18 AHENRY : function san_rdpm_tc_js_list_resources upgrade 7.1.2.1 new filter in resource selector in Timecard input
//
//***************************************************************************/
...
function san_rdpm_tc_js_list_resources() {
	// PC-3695
	// old: pre-7121 reuse of standard function _tc_js_list_resources and add RDPM filter
	//var user = context.applet.user;
	//var current_user = new vector();
	//current_user.push(user);
	//var result1 = plw._tc_build_resource_list_from_user(current_user).vector_copy();
	//var result1 = [];
	//var result2=new vector();
	//for (var res in result1 where res.SAN_UA_RDPM_B_FILTER_RESOURCE_LIST_FOR_TC_INPUT) {
	//	result2.push(res);
	//}
	// new: post-7121 using redefinition of getResourcesOfUser
	var _ht = new hashtable();
	san_getResourcesOfUser(context.applet.user, _ht, undefined, false);
	var result1 = _ht.keys;
	var result2 = [];
	for (var res in result1 where res.SAN_UA_RDPM_B_FILTER_RESOURCE_LIST_FOR_TC_INPUT) {
		result2.push(res);
	}
