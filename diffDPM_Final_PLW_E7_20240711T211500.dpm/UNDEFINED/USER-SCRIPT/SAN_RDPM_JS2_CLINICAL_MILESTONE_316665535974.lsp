...
(JAVASCRIPT::USER-SCRIPT
 :OBJECT-NUMBER 316665535974
 :NAME "SAN_RDPM_JS2_CLINICAL_MILESTONE"
 :COMMENT "Clinical Milestone Functions"
 :ACTIVE T
 :DATASET 118081000141
 :EVAL-ON-LOAD T
 :LOAD-ORDER 0
 :SCRIPT-CODE "namespace _rdpm_clinical_milestones;
// V2.3 Renamed san_rdpm_get_clinical_milestone_date into san_rdpm_get_clinical_milestone_date_and_onb, in order to return MS date and onb in a vector - PC-6758 - EPA GSE - 16-NOV-23
// V2.2 Modification of variable to identify vector instead - PC-6504 - EPA GSE - 17-MAY-23
// V2.1 Addtion of new Coutrny phase for Clinical Milestones - PC-6504 - GSE - 19-JAN-2023
...
function san_rdpm_get_clinical_milestone_date_and_onb(act,clin_ms_rule)
{
	var date = -1;
	var ms_onb_string = \"\";
	var result_vect=new vector();
				...
				with(filter.fromobject())
				{
					if (operation==\"Increase\")
					{
						for (var vAct in plc.workstructure /*where vAct.get(date_slot)!=undefined*/ order by [sort_date_rule])
						{
							...
							ms_onb_string=\"PRINT_NUMBER\".call(vAct.ONB,\"####\");
							v_break = true;
							break;
						}
					}
	...
	result_vect.push(date);
	result_vect.push(ms_onb_string);
	return result_vect;
}
...
function san_clinical_milestone_slot_reader(clin_ms_rule)
{
	...
	if (act instanceof plc.workstructure)
	{
		...
		if ((level==\"STUDY\" && act.SAN_UA_RDPM_B_IS_A_STUDY) || (level==\"STEP\" && act.WBS_TYPE!=\"\" && act.WBS_TYPE.SAN_RDPM_UA_AT_B_IS_STEP) || (level==\"COUNTRY\" && act.WBS_TYPE!=\"\" && act.WBS_TYPE.SAN_RDPM_UA_AT_B_IS_COUNTRY))
		{
			var get_milestone_date_and_onb_result=san_rdpm_get_clinical_milestone_date_and_onb(act,clin_ms_rule);
			result=get_milestone_date_and_onb_result[0];
		}
		...
		// PC-xxxx Propagate the dates of the Clinical milestones under the activities of the country
		else if (level==\"COUNTRY\") {
			var att_name = \"SAN_RDPM_DA_CLIN_MS_\"+clin_ms_rule.NAME;
			if (act.SAN_RDPM_UA_N_ACT_COUNTRY_ONB!= \"\") {
				var o_study_act = plc.work_structure.get(act.SAN_RDPM_UA_N_ACT_COUNTRY_ONB);
				if (o_study_act instanceof plc.work_structure && o_study_act!=act && o_study_act.WBS_TYPE.SAN_RDPM_UA_AT_B_IS_COUNTRY) {return o_study_act.get(att_name);}
			}
		}
	}
	return result;
...
function san_rdpm_export_activity_clinical_milestone(clin_milestone,level,activity_vector,vect_rule_planned,vect_rule_actual,file)
{ 
	// Define common 	
	for (var act in activity_vector where act instanceof plc.workstructure)
	{
		...
		var planned_desc =\"\";
		var actual_desc =\"\";
		var activity_country=\"\";
		var activity_desc=\"\";
		
		// récupération du périmètre de l'activité
		var act_pharma=act.PROJECT.SAN_RDPM_B_RND_PHARMA_PROJECT;
		var act_vaccines=act.PROJECT.SAN_RDPM_B_RND_VACCINES_PROJECT;
		var perimeter=\"COMMON\";
		if (act_pharma) perimeter=\"PHARMA\";
		if (act_vaccines) perimeter=\"VACCINES\";
		
		// Retrieve the planned date
		var rule_planned=\"\";
		for (var rule_planned_obj in vect_rule_planned where rule_planned_obj.SAN_RDPM_UA_S_APPLY_TO==perimeter) {
			rule_planned=rule_planned_obj;
		}
		
		if (rule_planned!=\"\" && rule_planned instanceof plc.__USER_TABLE_SAN_RDPM_UT_CLIN_MS_RULES)
		{
			var get_milestone_date_and_onb_result=_rdpm_clinical_milestones.san_rdpm_get_clinical_milestone_date_and_onb(act,rule_planned);
			planned_date = get_milestone_date_and_onb_result[0];
			milestone_onb_string = get_milestone_date_and_onb_result[1];
			if (planned_date!=-1)
			{
		...
		var rule_actual=\"\";
		for (var rule_actual_obj in vect_rule_actual where rule_actual_obj.SAN_RDPM_UA_S_APPLY_TO==perimeter) {
			rule_actual=rule_actual_obj;
		}
		
		// Retrieve the actual date
		if (rule_actual!=\"\" && rule_actual instanceof plc.__USER_TABLE_SAN_RDPM_UT_CLIN_MS_RULES)
		{
			var get_milestone_date_and_onb_result=_rdpm_clinical_milestones.san_rdpm_get_clinical_milestone_date_and_onb(act,rule_actual);
			actual_date = get_milestone_date_and_onb_result[0];
			milestone_onb_string = get_milestone_date_and_onb_result[1];
			if (actual_date!=-1)
			{
		...
		if (planned_date_string!=\"\" || actual_date_string!=\"\")
		{
			...
			//new attributes to export for COUNTRY
			activity_country=act.printattribute(\"SAN_RDPM_UA_S_COUNTRY_STUDY_ACT\");
			//activity_desc=act.printattribute(\"SAN_RDPM_UA_ACT_ACTION_CSO\");
			// Add clinical milestone line to file
			
			
			//new_line=\"\\\"\"+clin_milestone+\"\\\"\"+file_separator+\"\\\"\"+level+\"\\\"\"+file_separator+\"\\\"\"+activity_id+\"\\\"\"+file_separator+activity_onb+file_separator+planned_date_string+file_separator+actual_date_string+file_separator+\"\\\"\"+planned_desc+\"\\\"\"+file_separator+\"\\\"\"+activity_country+\"\\\"\"+file_separator+\"\\\"\"+activity_desc+\"\\\"\";
			new_line=\"\\\"\"+clin_milestone+\"\\\"\"+file_separator+\"\\\"\"+level+\"\\\"\"+file_separator+\"\\\"\"+activity_id+\"\\\"\"+file_separator+activity_onb+file_separator+planned_date_string+file_separator+actual_date_string+file_separator+\"\\\"\"+planned_desc+\"\\\"\"+file_separator+\"\\\"\"+activity_country+\"\\\"\"+file_separator+milestone_onb_string;
...
function san_rdpm_bo_clinical_milestones_export()
{
	...
	ClinMSFile.writeln(\"\\\"CLINICAL_MILESTONE\\\",\\\"LEVEL\\\",\\\"ACTIVITY_ID\\\",\\\"ACTIVITY_ONB\\\",\\\"PLANNED_DATE\\\",\\\"ACTUAL_DATE\\\",\\\"SAN_RDPM_UA_S_FUNC_DESC\\\",\\\"SAN_RDPM_UA_S_COUNTRY_STUDY_ACT\\\",\\\"MS_ONB\\\"\");
	
	// Generate the list of studies and step to work on
	var study_list = new vector();
	var step_list = new vector();
	var country_list = new vector();
	
	// Loop on Project
	var list = [];
	// Loop on Projects: restrict loop to Pasteur Active Projects + addition of Pharma Project
	for(var o_project in plc.project where o_project.DELETED==false && (o_project.SAN_RDPM_B_RND_VACCINES_PROJECT || o_project.SAN_RDPM_B_RND_PHARMA_PROJECT) && o_project.STATE==\"Active\" && o_project._WZD_AA_B_PERMANENT == false)
	{
		list.push(o_project);
	}
	...
	with(filter.fromobject()){
		...
		
		{
			...
			else
			{
				...
				{
					if (o_act.WBS_TYPE!=\"\" && o_act.WBS_TYPE.SAN_RDPM_UA_AT_B_IS_COUNTRY && o_act.SAN_RDPM_UA_B_TASK_EXPORT_FILTER)
					{
						country_list.push(o_act);
					}
				}				
			}
		}
	...
	{
		var clin_ms_rule_study_planned = new vector();
		var clin_ms_rule_study_actual = new vector();
		var clin_ms_rule_step_planned = new vector();
		var clin_ms_rule_step_actual = new vector();
		var clin_ms_rule_country_planned = new vector();
		var clin_ms_rule_country_actual = new vector();
		...
		for (var clin_ms_rule in clin_ms.get(\"USER_ATTRIBUTE_INVERSE_SAN_RDPM_UA_S_CLIN_MS.__USER_TABLE_SAN_RDPM_UT_CLIN_MS_RULES\") where clin_ms_rule.SAN_RDPM_UA_B_EXPORT)
		{
			// Vaccines rules only
			{
				if (clin_ms_rule.SAN_RDPM_UA_S_LEVEL==\"STUDY\")
				{
					...
					{
						clin_ms_rule_study_planned.push(clin_ms_rule);
					}
					else
					{
						if (clin_ms_rule.SAN_RDPM_UA_S_MILESTONE_TYPE==\"ACTUAL\")
						{
							clin_ms_rule_study_actual.push(clin_ms_rule);
						}
					}
				}
				else
				{
					if (clin_ms_rule.SAN_RDPM_UA_S_LEVEL==\"STEP\")
					{
						if (clin_ms_rule.SAN_RDPM_UA_S_MILESTONE_TYPE==\"PLANNED\")
						{
							clin_ms_rule_step_planned.push(clin_ms_rule);
						}
						else
						{
							if (clin_ms_rule.SAN_RDPM_UA_S_MILESTONE_TYPE==\"ACTUAL\")
							{
								clin_ms_rule_step_actual.push(clin_ms_rule);
							}
						}
					}
					else
					{
						if (clin_ms_rule.SAN_RDPM_UA_S_LEVEL==\"COUNTRY\")
						{
							if (clin_ms_rule.SAN_RDPM_UA_S_MILESTONE_TYPE==\"PLANNED\")
							{
								clin_ms_rule_country_planned.push(clin_ms_rule);
							}
							else
							{
								if (clin_ms_rule.SAN_RDPM_UA_S_MILESTONE_TYPE==\"ACTUAL\")
								{
									clin_ms_rule_country_actual.push(clin_ms_rule);
								}
							}
						}
					}					
		...
		// Process Study
		if (clin_ms_rule_study_planned.length>0 || clin_ms_rule_study_actual.length>0)
		{
		...
		// Process Step
		if (clin_ms_rule_step_planned.length>0 || clin_ms_rule_step_actual.length>0)
		{
		...
		// Process Country
		if (clin_ms_rule_country_planned.length>0 || clin_ms_rule_country_actual.length>0)
		{
			san_rdpm_export_activity_clinical_milestone(clin_ms.NAME,\"COUNTRY\",country_list,clin_ms_rule_country_planned,clin_ms_rule_country_actual,ClinMSFile);
		}
	}
