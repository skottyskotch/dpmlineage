...
san_create_act_prime_gov_dynamic_attributes('SAN_RDPM_UA_ACT_ROOT_PRODUCT_CODE','SAN_UA_RWE_STUDY_PRODUCT','SAN_RDPM_DA_ACT_ROOT_PODUCT_CODE');
...
function san_main_vacc_region_slot_reader() {
    ...
    try {
        if(this.SAN_UA_RDPM_ACT_S_STUDY_ID != \"\") {
            var o_study_act = plc.workstructure.get(this.SAN_UA_RDPM_ACT_S_STUDY_ID);
            ...
            if(o_study_act instanceof plc.workstructure) {
                for(var s_vacc_region in o_study_act.SAN_RDPM_UA_S_LIST_REGIONS_VACC.parsevector()){
                    ...
                    var n_nbe_sites     = 0;
                    with([o_study_act.fromObject()]) {
                        for(var o_act_country in plc.workstructure where o_act_country.callBooleanFormula('WBS_TYPE IN (OC.SAN_RDPM_CS_AT_STUDY) AND SAN_UA_RDPM_PV_REG_VACC = ~s'.format(s_vacc_region))){
                            n_nbe_countries++;
