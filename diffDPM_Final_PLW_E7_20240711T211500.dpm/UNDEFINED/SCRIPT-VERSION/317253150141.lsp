...
function _rdpm_recreate_links(o_study, o_link_template)
{
	...
	with(o_study.fromObject()){
		for (var o_act in plc.work_structure){
			...
			if(o_act.WBS_TYPE.printattribute() in ['Recruitment','Treatment','FUP/Extension']){o_act.set('EXTND','Extendible');}
		}
	}
...
function _rdpm_auto_link_cohort_country(){
	...
	var o_study = v_selection[0];

	if (v_selection.length==1 && o_study instanceof plc.work_structure && o_study.SAN_UA_RDPM_B_IS_A_STUDY==true){
		...
		plw.alert('A study needs to be selected first!');
	}
}"
