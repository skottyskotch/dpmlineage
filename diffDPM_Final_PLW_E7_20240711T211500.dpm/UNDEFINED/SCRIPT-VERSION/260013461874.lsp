...
function san_rdpm_js_ph_default_value(role,act,default_val,act_field,wbs_form_field)
{
	...
	if (act_value!=undefined && act_value!=false && act_value.internal==false)
	{
		result = act_value;
	}
	else
	{	
		if (role.SAN_UA_RDPM_B_DISPLAYED_WBS_FORM_PHARMA){
			...
			var role_value=san_pjs_get_upper_level_value(act_onb,role_onb,wbs_form_field);
			if (role_value!=undefined && role_value!=false) result= role_value;
		}
	}
	return result;
}"
