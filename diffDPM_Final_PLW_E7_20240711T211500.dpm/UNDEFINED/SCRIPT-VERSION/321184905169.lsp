...
if (vPrez!=undefined)
{
	if (vPortfolio!=undefined)
	{
		...
		for (var vProj in vListProj)
		{
			...
			var vPDFpath =new pathname(vExpDirectory+vFileName);
		...
		if (vRepF!=undefined)
		{
			for (var vFra in vHashFr)
			{
				...
				var string vPDFpathFra=vExpDirectory+vFileNameFra;
				var vPDFfileFra=vRepF.print(vFra, \"PDF-PRINTER\", \"A3\", \"LANDSCAPE\", vPDFpathFra);
				plw.writetolog(\" -- PRINT MR ---- End printing Franchise \"+vFra.name+\" : \"+vPDFpathFra);
			}
		}
