...
function san_pjs_leader_defined_by_act(Defined_Act,Act,Role_ONB,field_overwrite)
{
	...
	{
			if (Act==Defined_Act) return true;
	}
	else
	{
		act_parent=Act.network;
			if (act_parent && act_parent!=false && act_parent.internal==false)  return san_pjs_leader_defined_by_act(Defined_Act,act_parent,Role_ONB,field_overwrite);
	}
	return result;
}"
