...
(JAVASCRIPT::SCRIPT-VERSION
 :OBJECT-NUMBER 260010664574
 :DATASET 118081000141
 :SCRIPT-CODE "namespace _wbs_form;

// Reader (Generic)
function san_wbs_form_slot_reader(role_onb,field,default_val) {
...
// Reader (Leader Site = Generic + Get the site of the leader)
function san_wbs_form_slot_reader_leader_site(role_onb,field,default_val) {
	var result=default_val;
	var leader_site=\"\";
    var act_onb=context.SAN_UA_TMP_CURRENT_ACT_WBS_FORM_ONB;
	
	var leader=san_pjs_get_upper_level_value(act_onb,role_onb,\"USER_ATTRIBUTE_SAN_UA_S_LEADER\");
	if (leader!=undefined && leader!=false && leader.internal==false)
	{
...
function san_wbs_form_parent_slot_reader(role_onb,field,default_val) {
    var result=default_val;
    var act_onb=context.SAN_UA_TMP_CURRENT_ACT_WBS_FORM_ONB;
...
// Modifier (Leader = Generic + Update leader on activities)
...
// Initializer
function san_wbs_form_init_number(value){return 1;}
function san_wbs_form_init_string(value){return \"\";}
function san_wbs_form_init_boolean(value){return false;}
function san_wbs_form_init_object(value){return \"\";}
...
function san_wbs_form_complexity_factor_slot_reader() {
	if (this.SAN_UA_RDPM_B_DISPLAYED_WBS_FORM_PHARMA )
		return san_wbs_form_slot_reader(this.ONB,\"USER_ATTRIBUTE_SAN_UA_N_COMPLEXITY_FACTOR\",1);
}
...
function san_pjs_create_dynamic_attribute_complexity_factor(){
    try{
		...
		Slot.initializer = san_wbs_form_init_number;
    }
    catch(error e){
...
function san_wbs_form_complexity_factor_parent_slot_reader() {
		return san_wbs_form_parent_slot_reader(this.ONB,\"USER_ATTRIBUTE_SAN_UA_N_COMPLEXITY_FACTOR\",1);
}
...
function san_pjs_create_dynamic_attribute_leader(){
    try{
        ...
        slot.hiddenInIntranetServer = false;
		Slot.initializer = san_wbs_form_init_object;
    }
    catch(error e){
        plw.writeln(\"Could not create slot due to error: \" + e);
    }
...
function san_wbs_form_leader_site_slot_reader() {
	if (this.SAN_UA_RDPM_B_DISPLAYED_WBS_FORM_PHARMA )
		return san_wbs_form_slot_reader_leader_site(this.ONB,\"USER_ATTRIBUTE_SAN_UA_S_LEADER_SITE\",\"ND\");
}
...
function san_wbs_form_excluded_res_slot_reader() {
	if (this.SAN_UA_RDPM_B_DISPLAYED_WBS_FORM_PHARMA )
		return san_wbs_form_slot_reader(this.ONB,\"USER_ATTRIBUTE_SAN_UA_B_EXCLUDED_RES\",false);
}
...
function san_pjs_create_dynamic_attribute_excluded_res(){
    try{
        ...
        slot.hiddenInIntranetServer = false;
		Slot.initializer = san_wbs_form_init_boolean;
    }
    catch(error e){
        plw.writeln(\"Could not create slot due to error: \" + e);
    }
...
function san_wbs_form_excluded_res_parent_slot_reader() {
    return san_wbs_form_parent_slot_reader(this.ONB,\"USER_ATTRIBUTE_SAN_UA_B_EXCLUDED_RES\",false);
}
...
try{
 ...
 :VERSION 6
 :_US_AA_D_CREATION_DATE 20201003000000
