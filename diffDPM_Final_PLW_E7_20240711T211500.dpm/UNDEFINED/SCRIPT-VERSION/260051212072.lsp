...
function san_rdpm_tc_filter_my_OBS_activities(f) {
	...
	var vActResult=new vector();
	//plw.alert(vRes);
	if(vRes != undefined && vRes instanceOf plc.Resource && vObs != undefined && vObs != \"\" && vObs instanceOf plc.obs_node)
	{
		var vResTcProfile=vRes.CURRENT_TIMECARD_PROFILE.printattribute();
		...
		if (vResTcProfile==\"PASTEUR\") {
			...
			if (vObs.obs_element!=\"\"){
            vActObsParent.push(vObs.obs_element);	}			
