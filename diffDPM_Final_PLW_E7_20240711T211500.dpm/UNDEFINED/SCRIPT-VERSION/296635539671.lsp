...
function san_rdpm_bo_planned_wkl_cost_export()
{
	...
	// Loop on each project to export
		...
		for (var o_project in v_proj_vect)
			...
			plw.writetolog(\"Processing Project: \"+o_project.name);
				...
				{
					for(var o_hes in plc.TIME_SYNTHESIS where ((o_hes.FD instanceof DATE) && !(o_hes.FD<start_horizon_date)))
						{
							var v_hes_string = \"\";
							...
							// define start date for computation
							...
							var v_DateVect = new plw.datevector(\"MONTH\",start_date_compute,o_hes.FD);
							...
							if (o_hes._PM_NF_B_BASIC_TYPE_HOURS){
								...
								var curve_eac_day = cost.computecurves(curvename, \"Day\", \"MONTH\", false, start_date_compute, o_hes.FD, o_hes);
								var curve_eac_fte = cost.computecurves(curvename, \"FTE (Full Time Equivalent)\", \"MONTH\", false, start_date_compute, o_hes.FD, o_hes);
							...
							if(hes_type == \"AUTO\" || hes_type == \"Hours\" || hes_type == \"MANUAL\" || hes_type == \"Expenditures\" || hes_type == \"Sites\" || hes_type == \"Subjects\"){
								...
								// Build EAC curves on 'Hours and expenditures' object
								var curve_eac_keuros = cost.computecurves(curvename, \"k€\", \"MONTH\", false, start_date_compute, o_hes.FD, o_hes);
	...
	var filepath = context.SAN_RDPM_UA_OC_S_EXPORT_SFTP_PATH;
...
plw.writeln(\"Exporting: Planned Workload & costs\");
...
plw.writeln(\"Exports: Success\");"
 ...
 :VERSION 22
 :_US_AA_D_CREATION_DATE 20210315000000
