		...
		if (ResSkill!= undefined) {
			...
			if (vResTcProfile==\"PHARMA\") {
				...
				if (ka instanceof KeyAttribute){
					with(ka.fromObject()){
						for (var talloc in plc.taskalloc where talloc.project.getinternalvalue(\"STATE\").ToString()==\"ACTIVE\") {
							total_plh++;
				...
				plw.writeln(\"Nb of scanned planned hours in active projects: \"+total_plh);
				plw.writeln(\"Nb of distinct activities: \"+total_distinct_act);
...
function san_rdpm_pjs_tcm_addActivityInTimesheet(id) {
 ...
 :VERSION 19
 :_US_AA_D_CREATION_DATE 20210304000000
