...
function san_wbs_form_slot_modifier_leader(nvalue,role_onb,field,default_val) {
	if (this.SAN_UA_RDPM_B_DISPLAYED_WBS_FORM_PHARMA)
	{
		...
		if (nvalue!=\"\")
		{
    		var leader_site=nvalue._RM_REVIEW_RA_LOCATION;
    		if (leader_site!=undefined && leader_site!=false && leader_site.internal==false)
    		{
    			result=san_pjs_set_wbs_form_data_value(act_onb,role_onb,\"USER_ATTRIBUTE_SAN_UA_S_LEADER_SITE\",leader_site);
    		} 
    		else
    		{
    		    result=san_pjs_set_wbs_form_data_value(act_onb,role_onb,\"USER_ATTRIBUTE_SAN_UA_S_LEADER_SITE\",\"ND\");
    		}
...
function san_wbs_form_sourcing_slot_modifier(nvalue) {
	if (this.SAN_UA_RDPM_B_DISPLAYED_WBS_FORM_PHARMA){
		...
		var act_onb=context.SAN_UA_TMP_CURRENT_ACT_WBS_FORM_ONB;
		var result=\"\";
		if (nvalue!=\"\" && nvalue.printattribute()!=\"INT\")
		{
		...
		else
		{
			...
			result=san_pjs_set_wbs_form_data_value(act_onb,role_onb,\"USER_ATTRIBUTE_SAN_UA_S_PROVIDER\",\"\");
