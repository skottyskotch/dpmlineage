...
//  Creation of the script to resolve the performance issue using Add activity button in timecard
//
//***************************************************************************/
	...
	if(vRes != undefined && vRes instanceOf plc.Resource && vObs != undefined && vObs instanceOf plc.obs_node)
	{
		...
		if (vResTcProfile==\"PHARMA\") {
			with(vObs.fromobject()) {
				for (var ObsAct in vObs.get(\"OBS_CHILDREN\")) {
					if (ObsAct instanceof plc.work_structure){
					var ObsActStudy=san_rdpm_get_study(ObsAct);
					if (ObsAct.OBS_ELEMENT==vObs && san_rdpm_check_act_can_be_inserted(ObsActStudy)) vActResult.push(ObsActStudy);
					}
				}
			}
		}
		...
		if (vResTcProfile==\"PASTEUR\") {
			...
			for (var vObsPart in vActObsParent){		
				with(vObsPart.fromobject()) {
					for (var vObsAct in vObsPart.get(\"OBS_CHILDREN\"))  {		
					   if (vObsAct instanceof plc.work_structure){
						  if ( san_rdpm_check_act_can_be_inserted(vObsAct)) vActResult.push(vObsAct);
					   }
					}
				}		
            }
			// add activity level 1 and obs =\"\"
		   ...
		   && prj._wzd_aa_b_permanent==false && prj.parent.printattribute()==\"\")
			 {
				 ...
				 if ( ActL1 instanceof plc.network && ActL1.obs_element.printattribute()==\"\")
				 {
				 vActResult.push(vObsPart);
				 }
			 }
...
function RDPM_tc_filter_all_activities(f) {	
...
function RDPM_tc_filter_global(f) {
	...
	if (context.SAN_RDPM_UA_CP_TC_PORT_CHOICE==cond1) {
		...
		if (context.SAN_RDPM_UA_CP_TC_PORT_CHOICE==cond2) {
			...
			if (context.SAN_RDPM_UA_CP_TC_PORT_CHOICE==cond3) {
				RDPM_tc_filter_all_activities(f);
			}
			else {
				return false;
			}
...
RDPM_global_TC_Context_Activity_Relation.mapmethod = RDPM_tc_filter_global;
