    		...
    		if (count_clin_ms_start>0)
            {
        		...
        		if (list_act!=undefined) 
        		{
        			...
        			for (var Act in vect_act)
        			{
        				...
        				if (act_obj instanceof plc.workstructure && act_obj.AS!=iStore_date)
        				{
        				    PS_Before=act_obj.PS;
        					act_obj.AS = iStore_date;
        					act_obj.SAN_RDPM_UA_S_SYSTEM_OWNER_ACTUAL_START = \"ISTORE\";
        					plw.writetolog(\"Update actual start for activity : \" + act_obj.printattribute() + \"[\"+act_obj.SAN_RDPM_UA_S_IMPACT_CODE_START_MS+\"] - PS before update :  \"+PS_Before+\" - New date : \"+ iStore_date);
        				}
        			}
        		}
            }
    		...
    		if (count_clin_ms_end>0)
            {
        		...
        		if (list_act!=undefined) 
        		{
        			...
        			for (var Act in vect_act)
        			{
        				act_onb=Act.parsenumber(\"####\");
        				act_obj=plc.workstructure.get(act_onb);
        				if (act_obj instanceof plc.workstructure && act_obj.AF!=iStore_date)
        				{
        				    PF_Before=act_obj.PF;
        					act_obj.AF = iStore_date;
        					act_obj.SAN_RDPM_UA_S_SYSTEM_OWNER_ACTUAL_FINISH = \"ISTORE\";
        					plw.writetolog(\"Update actual finish for activity : \" + act_obj.printattribute()+ \"[\"+act_obj.SAN_RDPM_UA_S_IMPACT_CODE_END_MS+\"] - PF before update : \"+PF_Before+\" - New date : \"+ iStore_date);
        				}
        			}
        		}
            }
