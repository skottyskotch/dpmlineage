...
function san_js2_insert_library() {
	...
	if (b_indication_selected==false && b_no_selection_done==false) {
		...
		// PC-3512 Check multiple activities from template
		var lib_onb = new hashtable();
		var lib_onb_duplicate = new hashtable();
		...
		with(o_actSelection.fromobject()){
			for (var o_actSyncTarget in plc.work_structure where o_actSyncTarget._PM_AA_N_LIBRARY_ACT_ONB!=0){
				//plw.alert(o_actSyncTarget.printattribute() + \" - \" +o_actSyncTarget._PM_AA_N_LIBRARY_ACT_ONB);
				if (lib_onb.get(o_actSyncTarget._PM_AA_N_LIBRARY_ACT_ONB)!=undefined)
				{
					lib_onb_duplicate.set(o_actSyncTarget._PM_AA_N_LIBRARY_ACT_ONB,\"1\");
				}
				else
				{
					lib_onb.set(o_actSyncTarget._PM_AA_N_LIBRARY_ACT_ONB,\"1\");
				}
			}
		}		
		
		with(o_actSelection.fromobject()){
			for (var o_actSyncTarget in plc.work_structure where o_actSyncTarget._PM_AA_N_LIBRARY_ACT_ONB!=0 && lib_onb_duplicate.get(o_actSyncTarget._PM_AA_N_LIBRARY_ACT_ONB)==undefined){
 ...
 :VERSION 13
 :_US_AA_D_CREATION_DATE 20210407000000
