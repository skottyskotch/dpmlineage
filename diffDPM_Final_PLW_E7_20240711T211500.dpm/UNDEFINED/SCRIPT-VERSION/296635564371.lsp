	...
	// Loop on each project to export
		...
		for (var o_project in v_proj_vect)
			...
			plw.writetolog(\"Processing Project: \"+o_project.name);
				...
				{
					for(var o_hes in plc.TIME_SYNTHESIS where ((o_hes.FD instanceof DATE) && !(o_hes.FD<start_horizon_date) && (o_hes.SD instanceof DATE) && !(o_hes.SD>end_horizon_date)))
						{
							var v_hes_string = \"\";
