...
function san_rdpm_tc_filter_my_OBS_activities(f) {
	...
	if(vRes != undefined && vRes instanceOf plc.Resource && vObs != undefined && !vObs.internal  && vObs instanceOf plc.obs_node)
	{
		...
		if (vResTcProfile==\"PASTEUR\") {
		   ...
		   && !prj._wzd_aa_b_permanent && prj.parent.index==\"\" )  // a tester
			 {
				 var ActL1=plw.top_level_ws(prj);
				 if ( ActL1 instanceof plc.network && san_rdpm_check_act_can_be_inserted(ActL1) && !ActL1.obs_element.internal)
