...
plw.writetolog(\"USER : \"+context.callstringformula(\"$CURRENT_USER\"));
...
for (var o_proj in plc.ordo_project where ( (o_proj.SAN_RDPM_B_RND_VACCINES_PROJECT || o_proj.SAN_RDPM_B_RND_PHARMA_PROJECT )  && o_proj._INF_NF_S_PRJ_STATE_INTERNAL == \"ACTIVE\" && o_proj._INF_NF_B_IS_TEMPLATE != true && o_proj._WZD_AA_B_PERMANENT != true )) 
{
    //SelectionVector.clear();
	var vector SelectionVector=new vector();
	plw.writetolog(\"Starting new2 : \"+ o_proj.NAME+\" - \"+o_proj.OPEN);
	...
	//for(var vAct in o_proj.ACTIVITIES)// where vAct.IS_A_LEAF==true || vAct.SAN_UA_RDPM_B_IS_A_STUDY==true)
	with(o_proj.fromobject())
	{
	for (var vAct in plc.workstructure)
	{
	    plw.writetolog(\"Act: \"+vAct.printattribute());
	    if (vAct.IS_A_LEAF==true || vAct.SAN_UA_RDPM_B_IS_A_STUDY==true) plw.writeln(\"OK\");
