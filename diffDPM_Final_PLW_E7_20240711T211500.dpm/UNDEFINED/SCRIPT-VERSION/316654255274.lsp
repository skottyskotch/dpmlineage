...
with(o_project_type_filter.fromobject())
{
    ...
    for(var o_project in plc.project where o_project.DELETED==false && o_project.STATE==\"Active\"  && o_project._WZD_AA_B_PERMANENT != true && o_project._INF_NF_B_IS_TEMPLATE != true )
    {
        list.push(o_project);
    }
}
