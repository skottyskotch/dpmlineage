...
{
    // Clinical milestone start
    if (act_type.SAN_RDPM_UA_S_CLIN_MS_START_CALC!=\"\")
    {
        list_ms_start.push(act_type);
		count_clin_ms_start++;
    }
    // Clinical milestone end
    if (act_type.SAN_RDPM_UA_S_CLIN_MS_END_CALC!=\"\")
    {
        list_ms_end.push(act_type);
		count_clin_ms_end++;
    }
...
if (s_pathFile.probefile())
{
    ...
    with(plw.no_locking)
    {
    	...
    	for (var o in vec_obj)
    	{
    		...
    		if (impact_country_code==undefined) impact_country_code=\"\";
        		...
        		if (list_act!=undefined) 
        		{
        			...
        			for (var Act in vect_act)
        			{
        				...
        				if (act_obj instanceof plc.workstructure && act_obj.AS!=impact_date && act_obj.wbs_type.SAN_RDPM_UA_B_ACTIVE_ISTORE_AD==true)
        				{
        					...
        					plw.writetolog(\"Update actual start for activity : \" + act_obj.printattribute() + \"[\"+act_obj.SAN_RDPM_UA_S_IMPACT_CODE_START_MS+\"] - PS before update :  \"+PS_Before+\" - New date : \"+ impact_date);
        				}
        			}
        		}
            }
        		...
        		if (list_act!=undefined) 
        		{
        			...
        			for (var Act in vect_act)
        			{
        				...
        				if (act_obj instanceof plc.workstructure && act_obj.AF!=impact_date && act_obj.wbs_type.SAN_RDPM_UA_B_ACTIVE_ISTORE_AD==true)
        				{
        					...
        					plw.writetolog(\"Update actual finish for activity : \" + act_obj.printattribute()+ \"[\"+act_obj.SAN_RDPM_UA_S_IMPACT_CODE_END_MS+\"] - PF before update : \"+PF_Before+\" - New date : \"+ impact_date);
        				}
        			}
        		}
            }
...
"
 ...
 :VERSION 15
 :_US_AA_D_CREATION_DATE 20211109000000
