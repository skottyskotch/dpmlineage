...
function san_pjs_get_wbs_form_data_object(act_onb,role_onb) {
   ...
   // PC-4166 : check file of activity and wbs form data 
    for (var wbs_form_data in act_obj.get(\"USER_ATTRIBUTE_INVERSE_SAN_UA_WBS_FORM_DATA_ACTIVITY.__USER_TABLE_SAN_RDPM_UT_WBS_FORM_DATA\") where wbs_form_data.file==act_obj.file) {
...
{
	if (object.file.SAN_RDPM_B_RND_PHARMA_PROJECT)
	{
		...
		if (role instanceof plc._RM_REVIEW_PT_ROLE)
		{
			...
			if (activity instanceof plc.workstructure && role.SAN_UA_RDPM_B_DISPLAYED_WBS_FORM_PHARMA)
			{
				...
				object._RM_REVIEW_RA_LOCATION=plw.san_pjs_get_wbs_form_string_value_on_object(activity.printattribute(),role.printattribute(),\"USER_ATTRIBUTE_SAN_UA_S_LEADER_SITE\");
			}
			else
			{
...
"
 ...
 :VERSION 36
 :_US_AA_D_CREATION_DATE 20210728000000
