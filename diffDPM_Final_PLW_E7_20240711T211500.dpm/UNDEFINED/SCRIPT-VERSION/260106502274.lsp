...
function san_wbs_form_slot_reader_leader_site(role_onb,field,default_val,empty_val){
	var result=\"\";
	var leader = san_wbs_form_slot_reader(role_onb,\"USER_ATTRIBUTE_SAN_UA_S_LEADER\",\"\",\"\");
	if (leader instanceof plc.resource && leader._RM_REVIEW_RA_LOCATION instanceof plc._RM_REVIEW_PT_LOCATIONS)
	{
		return  leader._RM_REVIEW_RA_LOCATION;
	}
	else
	{
		return san_wbs_form_slot_reader(this.ONB,field,default_val,empty_val);
	}
}
...
function san_wbs_form_sourcing_slot_modifier(nvalue) {
	if (this.SAN_UA_RDPM_B_DISPLAYED_WBS_FORM_PHARMA){
		...
		if (nvalue!=\"\")
		{
			...
			if (nvalue.printattribute()==\"INT\" || (provider!=undefined && provider!=false && provider!=\"\"))
			{
...
function san_wbs_form_provider_slot_modifier(nvalue) {
	if (this.SAN_UA_RDPM_B_DISPLAYED_WBS_FORM_PHARMA)
	{
		...
		if (Sourcing!=\"\" && Sourcing.printattribute()!=\"INT\" && nvalue==\"\")
		{
...
"
 ...
 :VERSION 20
 :_US_AA_D_CREATION_DATE 20201107000000
