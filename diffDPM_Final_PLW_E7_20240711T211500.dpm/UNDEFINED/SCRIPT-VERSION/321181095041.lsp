...
//  Modification - ABE - 10/02/2022 : PC-5547 - Modification of san_rdpm_team_member_from_project to take account propagation rule when the project Pharma or Vacccine + Indication (Level >1)
//***************************************************************************/
...
//PC-5547 : Take account propagation rule when the project Pharma or Vaccine + Indication  (level>1)
function san_rdpm_team_member_from_project(project_role)
{
    ...
    {
        //PC-5547 : Take account propagation rule for Pharma & Vaccine Project + Indication
        if (o_project.SAN_RDPM_UA_PRJ_RND_IND_VAC || o_project.SAN_RDPM_UA_PRJ_RND_PRJ) {
            
                return san_rdpm_team_member_from_project_parent(obj_project_role,o_project);
        }
        else if (o_project.SAN_RDPM_UA_PRJ_RND_PRJ_VAC || o_project.SAN_RDPM_UA_PRJ_RND_IND) {
			// PC-4571 - For indications, merge the values of Project + Indication
			var team_indication =  san_rdpm_team_member_from_project_parent(obj_project_role,o_project);
			var team_project = san_rdpm_team_member_from_project_parent(obj_project_role,o_project.PARENT);
			var global_team = \"LIST_MERGE\".call(team_indication,team_project);
            return global_team;
...
// PC-3008 Dynamic attribute \"Long absence?\" on Module class
// Used to check if all Elearning MCQ visible by the current user are complete
// and overall average score is equal or higher than module score target
		...
		with(oModule.fromobject()) {
			...
			for(var SubjectRelation in oModule.get(\"r._ELEARN_RA_MODULE._ELEAR_PT_SUBJ_MOD_NN\")){
				...
				if(Subject InstanceOf plc._GUI_PT_ELEARNING){
					//We get the display condition to check which elearning should be included
					var sDisplayCondition = Subject._ELEARN_ASA_S_DISPLAY_CONDITION;
					if(Subject.callBooleanFormula(sDisplayCondition)){
						var oMcq = Subject._ELEARN_RA_MCQ;
						if(oMcq instanceof plc._ELEARN_PT_MCQ && oMcq.NAME !=\"\"){
							nSumPercentComplete = nSumPercentComplete + oMcq._ELEARN_DA_N_SCORE_FIELD;
							nMcqComplete++;
						}
					}
				}
			...
			if(nMcqComplete>0){
				//Compute average percent complete
				nPercentComplete=nSumPercentComplete/nMcqComplete;
				//Confirm score reached if nPercentComplete > module requiered score
				if(nPercentComplete >= math.round(oModule._GUI_AA_N_MCQ_PERCENTAGE * 100)){
...
try{
 ...
 :VERSION 56
 :_US_AA_D_CREATION_DATE 20220211000000
