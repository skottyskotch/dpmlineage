        ...
        while(s_line != undefined){
			...
			if(b_first_line==false){
				...
				for(var i=0 ; i<v_DateVect.length-1 ; i++){
					...
					if (n_load!=0 && o_activity instanceof plc.work_structure && o_res instanceof plc.resource && o_skill instanceof plc.resource_skill && o_role instanceof plc._RM_REVIEW_PT_ROLE){
...
function san_rdpm_cso_fte_upload()
{
 ...
 :VERSION 10
 :_US_AA_D_CREATION_DATE 20210909000000
