...
//  Script contaitning the functions to export and import CSO FTE costs
//
//***************************************************************************/
...
function san_rdpm_cso_fte_costs_export()
{
	...
	var s_filepath = '/tmp/';
	var s_cso_fte_costs_filename = 'CSO_FTE_COSTS.csv';
	var o_cso_fte_costs_filepath = new plw.pathname(s_filepath+s_cso_fte_costs_filename);

	if(o_cso_fte_costs_filepath.probefile())
	{
		if (plw.question('The file \"CSO_FTE_COSTS.csv\" already exists! Do you want to replace it?')) {o_cso_fte_costs_filepath.deletefile();}
		else {return 0;}
	}
...
function san_rdpm_cso_fte_costs_import()
{
 ...
 :VERSION 0
 :_US_AA_D_CREATION_DATE 20210511000000
