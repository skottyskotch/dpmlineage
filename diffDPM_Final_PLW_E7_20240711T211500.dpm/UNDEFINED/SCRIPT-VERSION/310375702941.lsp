...
//  Fix dates export (PC-4385), upload cancellation (PC-PC-4411) and add filters to export active pharma projects only (PC-4384)
//
...
function san_rdpm_cso_fte_export()
{
	...
	for(var o_project in plc.ordo_project where o_project.SAN_RDPM_B_RND_PHARMA_PROJECT == true && o_project.SAN_UA_RWE_PROJECT_CODE_PRIME!='' && o_project.PARENT.printattribute()=='' && o_project.getinternalvalue(\"STATE\").toString()=='ACTIVE') {v_projects.push(o_project);}
...
function san_rdpm_cso_fte_upload()
{
 ...
 :VERSION 7
 :_US_AA_D_CREATION_DATE 20210901000000
