...
//  Modification V1 2020/10/06 SAK  : Add PC-686 only obs of the resource and the parent obs of the resource
//
//***************************************************************************/
...
function san_rdpm_tc_filter_my_role_activities(f) {
	...
	if(vRes != undefined && vRes instanceOf plc.Resource) {
		...
		// Warning : hard coded \"Financial task\". Prefer plug it on a setting , waiting to decide how we will manage settings for rdpm
		...
		if (ResSkill!= undefined) {
			...
			if (vResTcProfile==\"PHARMA\") {
						...
						for (var talloc in plc.taskalloc) {
							...
							if (TC_projects_table.get(MyRolePrj)==true) {
								if (act_table.get(MyRoleAct)==undefined) {
									...
									if (MyRoleActStudy!=undefined && study_table.get(MyRoleActStudy)==undefined) {
										total_distinct_std++;
										study_table.set(MyRoleActStudy,true);
										if (san_rdpm_check_act_can_be_inserted(MyRoleActStudy)) {
											vActResult.push(MyRoleActStudy);
				...
				plw.writeln(\"\");
				plw.writeln(\"############# Scanned insertable studies for skill \"+SkillString+\" #########\");
				plw.writeln(\"Nb of scanned planned hours: \"+total_plh);
				plw.writeln(\"Nb of distinct activities in TC projects: \"+total_distinct_act);
				plw.writeln(\"Nb of distinct studies: \"+total_distinct_std);
				plw.writeln(\"Nb of insertable studies: \"+total_std_insertable);
				plw.writeln(\"Nb of financial tasks: \"+total_distinct_ft);
				plw.writeln(\"Nb of insertable financial tasks: \"+total_ft_insertable);
				plw.writeln(\"TOTAL of activities to display in dialog: \"+vActResult.length);
				plw.writeln(\"######################\");
			}
			...
			//plw.alert(\"avant: \"+vActResult.length);
			//vActResult=vActResult.removeduplicates();
			//plw.alert(\"apres: \"+vActResult.length);
...
function san_rdpm_tc_filter_my_OBS_activities(f) {
	...
	if(vRes != undefined && vRes instanceOf plc.Resource && vObs != undefined && !vObs.internal  && vObs instanceOf plc.obs_node)
	{
		...
		if (vResTcProfile==\"PHARMA\") {
			...
			plw.writeln(\"\");
			plw.writeln(\"############# Scanned insertable studies for OBS \"+vObs+\" #########\");
			plw.writeln(\"Nb of distinct activities in TC projects: \"+total_distinct_act);
			plw.writeln(\"Nb of distinct studies: \"+total_distinct_std);
			plw.writeln(\"Nb of insertable studies: \"+total_std_insertable);
			plw.writeln(\"TOTAL of activities to display in dialog: \"+vActResult.length);
		...
		if (vResTcProfile==\"PASTEUR\") {
			...
			//PC-686 only obs of the resource and the parent obs of the resource
			//vActObsParent.push(vObs);
			var Vobsparent=vObs.obs_element;
			if (!vObsparent.internal){
				//vActObsParent.push(vObsparent);	
				with(vObsparent.fromobject()) {
					//for (var vObsAct in plc.work_structure where (vObsAct.OBS_ELEMENT==Vobs || vObsAct.OBS_ELEMENT==Vobsparent))  {
			...
			for ( var prj in plc.ordo_project where prj.san_rdpm_b_rnd_vaccines_project  && prj.getinternalvalue(\"STATE\").ToString()==\"ACTIVE\" 
			&& ! prj._wzd_aa_b_permanent && prj.parent.printattribute()==\"\")
			{
				var ActL1=plc.network.get(prj.name);
				// if ( ActL1 instanceof plc.network && ActL1.obs_element==\"\" && san_rdpm_check_act_can_be_inserted(ActL1))
				if ( ActL1 instanceof plc.network && ActL1.obs_element.internal && san_rdpm_check_act_can_be_inserted(ActL1))
...
function san_rdpm_init_table_key_plh_skill() {
	for (var ResSkill in plc.resourceskill where ResSkill.SAN_RDPM_UA_B_IS_MAIN_SKILL) {
		...
		plw.writeln(\"## Initialized table key for skill \"+SkillString+\" --> \"+total_plh+\" planned hours\");
	}
}
// initilize tk
//san_rdpm_init_table_key_plh_skill();"
 ...
 :VERSION 20
 :_US_AA_D_CREATION_DATE 20210305000000
