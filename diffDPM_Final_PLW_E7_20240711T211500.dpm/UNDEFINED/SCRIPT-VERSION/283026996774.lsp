...
(JAVASCRIPT::SCRIPT-VERSION
 :OBJECT-NUMBER 283026996774
 :DATASET 118081000141
 :SCRIPT-CODE "namespace _san_lib;

function san_rdpm_pjs_create_sync_links(selection)
{
	var building_block;
	var act_sync;
	var sync_act_type;
	var act_target;
	var inserted_act=new hashtable();
	var nb_act=0;
	var result=\"\";
	...
	with(selection.fromobject()){
		...
		{
			inserted_act.set(vAct._PM_AA_N_LIBRARY_ACT_ONB,vAct);
		}
	}
	...
	with(selection.fromobject()){
		...
		{
			act_target=\"\";
			act_sync= vAct.SYNCHRONIZE_WITH;
			nb_act=0;
			
			// Case 1 -- Search link inside the inserted block
			if (inserted_act.get(act_sync._PM_AA_N_LIBRARY_ACT_ONB) instanceof plc.workstructure) 
			{
				act_target=inserted_act.get(act_sync._PM_AA_N_LIBRARY_ACT_ONB);
			}
			// Case 2 -- External links inside building block
			else
			{
				...
				if (building_block instanceof plc.workstructure && sync_act_type instanceof plc.WBS_TYPE)
				{
					...
					if (nb_act>1)
					{
						plw.alert(\"External synchronisation link : there is more than one activity with activity type \" + sync_act_type.name + \" in the building block \" + building_block.name + \". Impossible to recreate synchronisation link for \" + vAct.NAME +\".\"); 
						act_target=\"\";
					}
				}	
			}
			
			vAct.SYNCHRONIZE_WITH=act_target;
			if (nb_act<2 && act_target==\"\")
				plw.alert(\"The \\\"Synchronize with\\\" activity for \\\"\"+vAct.NAME+\"\\\" was not found. Field emptied!\");
			else
			{
				for (var o_syncRule in plc.SYNCHRONIZATION_RULE order by [[\"INVERSE\",\"PRIORITY\"]]){
					if (vAct.callbooleanformula(o_syncRule.FILTER)) {vAct.LAST_SYNC_RULE=o_syncRule.printattribute();break;}
				}
			}
		}
	}
...
function san_js2_insert_library() {
	...
	if (b_indication_selected==false && b_no_selection_done==false) {
		...
		//san_rdpm_pjs_create_sync_links(o_actSelection);
		...
		with(o_actSelection.fromobject()){
			...
			for (var o_actSyncSource in plc.work_structure where o_actSyncSource.SYNCHRONIZE_WITH instanceof plc.work_structure){
				if (o_actSyncSource.SYNCHRONIZE_WITH.PROJECT.SAN_RDPM_UA_PRJ_S_ROOT_PROJECT!=o_actSyncSource.PROJECT.SAN_RDPM_UA_PRJ_S_ROOT_PROJECT && o_actSyncSource.SYNCHRONIZE_WITH.PROJECT._INF_NF_B_IS_TEMPLATE){
					// Check if the link can be find inside the building block (Pharma project only)
					if (true)
					{
						var act_target=\"\";
						var nb_act=0;
						...
						if (building_block instanceof plc.workstructure && sync_act_type instanceof plc.WBS_TYPE)
						{
							...
							if (nb_act>1)
							{
								plw.alert(\"External synchronisation link : there is more than one activity with activity type \" + sync_act_type.name + \" in the building block \" + building_block.name + \". Impossible to recreate synchronisation link for \" + o_actSyncSource.NAME +\".\"); 
								act_target=\"\";
							}
						}	
