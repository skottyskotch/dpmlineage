    	...
    	for (var o in vec_obj)
    	{
    		...
    		if (list_act!=undefined) 
    		{
    			...
    			for (var Act in vect_act)
    			{
    				...
    				if (act_obj instanceof plc.workstructure && act_obj.AS!=impact_date && act_obj.SAN_RDPM_UA_B_ACTIVE_ISTORE_AD==true)
    				{
    					act_obj.AS = impact_date;
    					act_obj.SAN_RDPM_UA_S_SYSTEM_OWNER_ACTUAL_START = \"ISTORE\";
    					plw.writeln(\"Update actual start for activity : \" + act_obj.printattribute() + \"[\"+act_obj.SAN_RDPM_UA_S_IMPACT_CODE_START_MS+\"] - New date : \"+ impact_date);
    				}
    			}
    		}
    		...
    		if (list_act!=undefined) 
    		{
    			...
    			for (var Act in vect_act)
    			{
    				act_onb=Act.parsenumber(\"####\");
    				act_obj=plc.workstructure.get(act_onb);
    				if (act_obj instanceof plc.workstructure && act_obj.AF!=impact_date && act_obj.SAN_RDPM_UA_B_ACTIVE_ISTORE_AD==true)
    				{
    					act_obj.AF = impact_date;
    					act_obj.SAN_RDPM_UA_S_SYSTEM_OWNER_ACTUAL_FINISH = \"ISTORE\";
    					plw.writeln(\"Update actual finish for activity : \" + act_obj.printattribute()+ \"[\"+act_obj.SAN_RDPM_UA_S_IMPACT_CODE_END_MS+\"] - New date : \"+ impact_date);
    				}
    			}
    		}
...
"
 ...
 :VERSION 10
 :_US_AA_D_CREATION_DATE 20211015000000
