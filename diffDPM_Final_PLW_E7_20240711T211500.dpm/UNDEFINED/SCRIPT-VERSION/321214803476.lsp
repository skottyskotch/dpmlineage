...
try{
	if( ! Context.SAN_RDPM_UA_B_OC_PREPROD_PROD_ENV){
		...
		for (var o_user in plc.opx2_user where o_user.callbooleanformula(\"?ADMIN = FALSE AND INACTIVE =FALSE AND SAN_RDPM_UA_B_USER_ADMIN_NON_PROD_ENV\") && o_user.GROUPS_LIST !=\"\")
		{					
				Group_List.clear();
...
plw.writeln(\"End _san_access_rgight_user .... \");
 ...
 :VERSION 3
 :_US_AA_D_CREATION_DATE 20220406000000
