...
app.get('/api/graph-data', (req, res) => {
	db.serialize(() => {
		db.all("SELECT ID, TYPE from nodes limit 500", (err, nodes) => {
			...
			db.all("with cte as (select * from nodes limit 500) select a.Id, SOURCE, TARGET, INATTRIBUTE, BYNAME from edges a join cte b on a.source = b.id join cte c on a.target = c.id;", (err, edges) => {
				if (err) {
					res.status(400).json({"error": err.message});
					return;
				}
