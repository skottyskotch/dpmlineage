...
app.get('/api/graph-data/nodes/table', (req,res) => {
	if (req.query.db && req.query.table) {
		let sQueryNodes = format(sNodesFromOneTable, req.query.table);
		let sQueryEdges = format(sEdgesFromOneTable, req.query.table);
		if (hDatabases[req.query.db] != undefined) {
			let db = hDatabases[req.query.db];
			db.serialize(() => {
				db.all(sQueryNodes, (err, _nodes) => {
        ...
        res.json({"error":"Please request a database '/api/graph-data/nodes/table'?db=databasename.db&TABLE=tableid"});
    }
});
